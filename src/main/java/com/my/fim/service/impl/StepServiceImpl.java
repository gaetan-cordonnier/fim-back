...
public class StepServiceImpl implements StepService {
    ...
    public StepDto getStepById(Long id) {
        Step step = stepRepository.findById(id).orElseThrow(() -> new NotFoundExceptionMessage(ConstantUtils.STEP_NOT_FOUND));
        return mapToDto(step);
    }

    @Override
    public StepDto updateStepById(StepDto stepDto, Long id) {
        Step step = stepRepository.findById(id).orElseThrow(() -> new NotFoundExceptionMessage(ConstantUtils.STEP_NOT_UPDATED));
    ...
    public void deleteStepById(Long id) {
        Step step = stepRepository.findById(id).orElseThrow(() -> new NotFoundExceptionMessage(ConstantUtils.STEP_NOT_DELETED));
